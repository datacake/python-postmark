...
class PMMail(object):
    ...
    def __init__(self, **kwargs):
        ...
        template_alias: alias of Postmark template. See: https://postmarkapp.com/support/article/1117-how-do-i-use-a-template-alias
        ...
        self.__user_agent = "Python/%s (python-postmark library version %s)" % (
            "_".join([str(var) for var in sys.version_info]),
            __version__,
        )
        ...
        try:
            ...
            self.__user_agent = "%s (Django %s)" % (
                self.__user_agent,
                "_".join([str(var) for var in VERSION]),
            )
    ...
    def _set_metadata(self, value):
        ...
        if value is None:
            ...
            for k, v in value.items():
                if (not isinstance(k, str) and not isinstance(k, int)) or (
                    not isinstance(v, str) and not isinstance(v, int)
                ):
    ...
    template_alias = property(
        ...
        lambda self: setattr(self, "_PMMail__template_alias", {}),
    )
    ...
    def _check_values(self):
        ...
        if not self.__api_key:
            ...
            raise PMMailMissingValueException(
                "Cannot send an e-mail without a sender (.sender field)"
            )
        elif not self.__to and not self.__bcc:
            raise PMMailMissingValueException(
                "Cannot send an e-mail without at least one recipient (.to field or .bcc field)"
            )
        elif (self.__template_id or self.__template_model or self.__template_alias) and not all(
            [any([self.__template_id, self.__template_alias]), self.__template_model]
        ):
            raise PMMailMissingValueException(
                "Cannot send a template e-mail without a both template_id and template_model set"
            )
        elif not any(
            [self.__template_id, self.__template_alias, self.__template_model, self.__subject]
        ):
            raise PMMailMissingValueException("Cannot send an e-mail without a subject")
        elif (
            not self.__html_body
            and not self.__text_body
            and not self.__template_id
            and not self.__template_alias
        ):
            raise PMMailMissingValueException(
                "Cannot send an e-mail without either an HTML or text version of your e-mail body"
            )
        if self.__track_opens and not self.__html_body:
            print(
                "WARNING: .track_opens set to True with no .html_body set. Tracking opens will not work; message will still send."
            )
    ...
    def to_json_message(self):
        ...
        if len(self.__custom_headers) > 0:
            ...
            for key, value in self.__custom_headers.items():
                cust_headers.append({"Name": key, "Value": value})
...
class PMBatchMail(object):
    ...
    def __init__(self, **kwargs):
        ...
        acceptable_keys = ("api_key", "messages")
...
class PMBounceManager(object):
    ...
    def _check_values(self):
        ...
        if not self.__api_key:
            raise PMMailMissingValueException(
                "Cannot check bounces without a Postmark Server API Key"
            )
